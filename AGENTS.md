...
- `v0.9.0-alpha` 已新增只读章节导出、创作闭环清单、CLI 续写入口、显式 `POST /api/jobs/resume-continue` 与 `selected_worldline.json` 选择记录；推荐世界线可从前端生成下一章，也可「设为起点」并在工作台读回。
- 仍未做云端多用户持久队列、对象存储、向量库、overlay 驱动下一轮 runner 自动消费、运行后审计写回。
