...
| 分类 | 当前口径 |
| --- | --- |
| 已闭环支撑层 | v0.7-v1.0-local、运行前体检至 Graph/长期记忆 mock 复核链、真实 retrieval provider 和 opt-in Vector Retrieval Pipeline 均为已收口支撑层 |
...
| 标签 | 含义 | 是否能派生下一刀 |
| --- | --- | --- |
| 当前事实 | 最新完成度、验证基线和真实缺口 | 可以 |
| 当前主线 | 世界沙盘产品能力深化 | 可以 |
| 路线/阶段 | 解释阶段边界和候选触发条件 | 需回到 `memory.md` 复核 |
| 历史归档 | 已完成或历史快照 | 不直接派生 |
| 支撑层/后置 | provider、Graph、检索、发行、商业化等 | 仅用户明确点名时 |
| 研究/资产 | 论文、品牌、原型图 | 只作参考 |
