...
| v0.9.0-alpha | Long Novel Creation Loop | 长篇产品化闭环成立 | 进行中：Chapter Export / Chapter Collection Export / Checklist / Continuation Hint / Resume Continue HTTP Job / Worldline Selection Persistence / Post-run Audit Entry 已走通；上传/创建 -> 记忆 -> 分支运行 -> 审计 -> 选择世界线 -> 导出整体闭环仍是 alpha。 |
...
**测试基线**：`cd engine && python -m pytest -q` → **607 passed**（截至 2026-05-31，v0.9.0-alpha Chapter Collection Export 子刀后完整回归通过）；`cd engine/ui && pnpm run build` 通过。
...
> **仍未整体收口**：公开分享、版权工作流、provider/cost gateway 均未进入本子刀；运行后审计目前是只读入口，尚未写回正史账本或驱动下一轮 runner。
...
v0.1.2 resume continue
  ...
  -> v0.9.0-alpha Long Novel Creation Loop（进行中：Export / Collection Export / Checklist / Hint / Resume Job / Selection / Post-run Audit 已收口）
```
...
| P9.0-alpha | v0.9.0-alpha Long Novel Creation Loop | 上传 -> 记忆 -> 分支运行 -> 审计 -> 选择世界线 -> 导出 | 进行中：Export / Collection Export / Checklist / Hint / Resume Job / Selection / Post-run Audit 已收口 |
