...
| v0.9.0-alpha | Long Novel Creation Loop | 长篇产品化闭环成立 | 进行中：Chapter Export / Checklist / Continuation Hint 已走通；上传/创建 -> 记忆 -> 分支运行 -> 审计 -> 选择世界线 -> 导出整体闭环仍是 alpha。 |
...
**测试基线**：`cd engine && python -m pytest -q` → **599 passed**（截至 2026-05-31，v0.9.0-alpha Creation Loop Checklist 子刀后完整回归通过）；`cd engine/ui && pnpm run build` 通过。
...
> **仍未整体收口**：世界线选择持久化、运行后审计写回、自动继续生成下一章、多章节合集导出、公开分享、版权工作流、provider/cost gateway 均未进入本子刀。
...
v0.1.2 resume continue
  ...
  -> v0.9.0-alpha Long Novel Creation Loop（进行中：Export / Checklist / Hint 已收口）
```
...
| P9.0-alpha | v0.9.0-alpha Long Novel Creation Loop | 上传 -> 记忆 -> 分支运行 -> 审计 -> 选择世界线 -> 导出 | 进行中：Export / Checklist / Hint 已收口 |
