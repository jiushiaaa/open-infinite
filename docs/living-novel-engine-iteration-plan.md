...
| 阅读入口产品化 | `dossier-reading`、卷宗阅读页、`readable_entry`、世界线/检查点/角色/事件跳转已完成第一版 | 继续做正文锚点、误会图谱、阅读进度、独立角色/势力卷 |
| 作者链路产品化 | Reviewer 片段级建议 -> 作者勾选采纳 -> `edited_final_chapter` -> 确认入卷 -> 下一轮入口已完成第一版 | 继续做整章风格润色、可回滚对照、真实模型编辑器 |
| 世界入口与导航第一版 | 世界书架、顶栏世界内卷宗导航、锚定到天命书、沙盘空态导引、机制档案降噪、入口首屏 QA、世界内导览层 `WorldRunway`、锚定页启动卡、沙盘运行导览、移动端世界卷宗导航盘和移动端栏目保功能已完成第一轮 | 继续做更完整的世界内部壳、角色/势力独立卷、事件详情、跨页面视觉 QA 和状态提示 |
...
2. **长正文与连续阅读质量**
   - 已有 `continuous_reading_chapter` v2、卷宗阅读页、确认稿阅读链和自演可读入口。
   - 下一步做更自然的长文节奏、正文内证据锚点、误会图谱、跨章伏笔回收和真实文风稳定。

3. **语义 Reviewer 与整章风格润色**
   - 已有语义 Reviewer、局部改写建议、作者勾选采纳、编辑后定稿和自动确认入卷。
...
5. **世界内部卷宗壳**
   - 已有世界线页、检查点回放页、卷宗阅读页、世界沙盘页、顶栏世界内卷宗导航和 `WorldRunway` 导览层第一版。
   - 下一步做 `WorldWorkspaceShell`、角色/势力独立页、事件详情页和机制档案页，把旧工程面板收纳为支撑层。
