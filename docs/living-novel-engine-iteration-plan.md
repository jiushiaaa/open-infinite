...
**测试基线**：`cd engine && python -m pytest -q` → **702 passed**（截至 2026-06-01，v1.0-beta Auth Boundary Checklist-U 收口后完整回归通过）；`cd engine/ui && pnpm run build` 通过。
...
> **v1.0-beta Auth Boundary Checklist-U 已收口（2026-06-01）**：新增 `get_auth_boundary_checklist()` 与 `GET /api/settings/auth-boundary`，设置抽屉新增「认证边界清单」只读区，聚合本地单人账号语义、owner/editor/viewer 权限草案、请求级 ACL 缺口、项目空间映射和部署观测边界。该刀只读解释认证边界，不创建真实用户、不接认证 provider、不执行 ACL 拦截、不写 artifact、不打外网。收口归档见 `docs/completed/v1.0-beta-auth-boundary-checklist-u.md`。后续继续拆分对象存储 adapter 或配额执行边界小刀。
