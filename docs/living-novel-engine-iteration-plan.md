...
| v0.9.1-v0.9.4 | Provider/Cost、MasterSetting、Graph Memory、Advanced Runner | 真实使用压力增强 | v0.9.1 provider 摘要、usage、设置展示、手动估算与只读路由矩阵已完成；下一步收口核对，后续按成本、召回、设定管理、runner 复杂度触发，不提前重依赖。 |
...
**测试基线**：`cd engine && python -m pytest -q` → **626 passed**（截至 2026-06-01，v0.9.1 Route Matrix-E 后完整回归通过）；`cd engine/ui && pnpm run build` 通过。
...
> **v0.9.1 Route Matrix-E 已落地（2026-06-01）**：`GET /api/settings/providers` 新增只读 `routes`，列出读者干预生成、主题创世、导入抽取、视觉资产生成四个入口的 provider、mode、runner 与 fallback；设置抽屉「模型与用量状态」新增路由矩阵行。该子刀只解释当前路由，不新增写入开关、不改默认 mock / Seedream / runner 调用链；设置测试保持 25 passed，前端 build 通过。后续做 v0.9.1 收口核对。
...
| P9.1 | v0.9.1 Provider & Cost Gateway Lite | 多 provider 配置、模型路由、成本/用量估算、失败回退、Key 脱敏展示 | 进行中：Provider Gateway Summary-A / Provider Usage Summary-B / Provider Status Panel-C / Manual Price Estimate-D / Route Matrix-E 已收口，下一步收口核对 |
...
v0.7.1 Intervention Compiler（自由输入转抽象干预 + 动态分支轴，已完成）
  ...
  -> v0.8.10-B Runner State Execution MVP（已收口：最小 opt-in 状态写入）
  -> v0.9.0-alpha Long Novel Creation Loop（已整体收口，见 docs/completed/v0.9.0-alpha-long-creation-loop.md）
  -> v0.9.1 Provider & Cost Gateway Lite（进行中：provider 摘要/usage/设置展示/手动估算/路由矩阵已收口，下一步收口核对）
