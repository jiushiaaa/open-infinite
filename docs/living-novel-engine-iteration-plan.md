...
**测试基线**：`cd engine && python -m pytest -q` → **704 passed**（截至 2026-06-01，v1.0-beta Object Storage Boundary Checklist-V 收口后完整回归通过）；`cd engine/ui && pnpm run build` 通过。
...
> **v1.0-beta Object Storage Boundary Checklist-V 已收口（2026-06-01）**：新增 `get_object_storage_boundary_checklist()` 与 `GET /api/settings/object-storage-boundary`，设置抽屉新增「对象存储边界」只读区，聚合本地 artifact 盘点、资源映射、原文/holdout 私有隔离、项目保留策略输入、身份绑定和本地部署护栏。该刀只读解释对象存储 adapter 前置边界，不创建 bucket、不生成签名 URL、不上传文件、不写 artifact、不打外网。收口归档见 `docs/completed/v1.0-beta-object-storage-boundary-checklist-v.md`。后续继续拆分配额执行边界小刀。
