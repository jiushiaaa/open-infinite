...
**测试基线**：`cd engine && python -m pytest -q` → **683 passed**（截至 2026-06-01，v1.0-beta State Execution Audit Hook-N 收口后完整回归通过）；`cd engine/ui && pnpm run build` 通过。
