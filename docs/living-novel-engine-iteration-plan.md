...
| v0.9.1-v0.9.4 | Provider/Cost、MasterSetting、Graph Memory、Advanced Runner | 真实使用压力增强 | v0.9.1 与 v0.9.2 已整体收口；下一步复核 v0.9.3 图记忆评估触发条件，再按召回和 runner 复杂度触发，不提前重依赖。 |
...
**测试基线**：`cd engine && python -m pytest -q` → **632 passed**（截至 2026-06-01，v0.9.2 MasterSetting Workspace Lite 收口后完整回归通过）；`cd engine/ui && pnpm run build` 通过。
...
v0.7.1 Intervention Compiler（自由输入转抽象干预 + 动态分支轴，已完成）
  ...
  -> v0.9.3 Graph Memory Evaluation Spike（触发条件复核：召回不足时才评估 Zep/图数据库）
