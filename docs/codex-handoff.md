...
下一步应继续深化：真实 LLM 多 Agent 策略博弈、长正文/连续阅读质量、跨章误会回收、多事件索引深化、更强真实语义 Reviewer、整章风格润色和世界状态长期发酵。
...
| 已完成第一版 | S1-S9 世界沙盘链路、导入/创世开卷中枢、卷宗阅读页、卷宗阅读正文证据锚点/阅读进度/误会图谱、自演结果可读入口、Reviewer 局部重写采纳、编辑后定稿、确认入卷反哺下一轮入口、天命书首屏宪法封面、世界线首屏工作流中枢、检查点首屏醒来回放中枢、多视角首屏工作流中枢、机制档案首屏中枢、世界内导览层 `WorldRunway`、作者采纳台工作流中枢、锚定页启动卡与世界卷宗总览、角色个人卷独立页、势力卷独立页、事件多视角详情页、跨事件长线卷、沙盘运行导览、移动端世界卷宗导航盘与移动端保功能布局 |
...
- `LonglineReadingPage` / `GET /api/stories/<slug>/worldlines/<worldline_id>/longline-reading` / `#/world/<slug>/worldlines/<worldline_id>/longline` 已把连续阅读场景、卷宗、确认入卷、事件信息差和证据链组织成跨事件长线卷；顶栏、世界线页、卷宗阅读页和事件详情页都能进入。
...
- 长正文读感、跨章误会回收、跨章伏笔回收、长线阅读进度和真实文风一致性。
- Reviewer 整章风格润色、可回滚对照、真实模型编辑器和定稿质量门。
- `consequence_state` 长期状态机、人工确认、跨章节发酵和真实 LLM 决策消费。
- `WorldWorkspaceShell`、跨章角色/势力长线阅读、多事件索引深化、长线阅读进度和机制档案分区。
