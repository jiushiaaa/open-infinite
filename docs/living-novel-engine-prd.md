...
| v0.9.1 | Provider & Cost Gateway Lite | 多 provider 配置、模型路由、成本/用量估算、失败回退、Key 脱敏展示 | 进行中：provider 摘要、usage 聚合、设置展示、手动估算和只读路由矩阵已收口，后续做整体收口核对；不默认接 Zep / 图数据库 / OASIS / CAMEL / LangGraph |
