...
| v0.9.3 | Graph Memory Evaluation Spike | 评估 Zep / 图数据库 / GraphRAG 是否替换或增强现有 `canon_ledger` + BM25 + entity aliases | 进行中：Trigger-A 已新增只读触发报告，下一步 Retrieval Probe-B；仅在评测证明召回不足时进入重依赖评估 |
