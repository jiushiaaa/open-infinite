...
- **Creation Loop Closeout API**：`GET /api/stories/<slug>/creation-loop-closeout` 直接返回 `completion_status`、`actions` 与 `closeout`，供真实样例或导入项目做自动验收和阻塞补齐；`select_worldline` action 带可直接 POST 的 payload。
