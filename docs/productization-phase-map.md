...
| v1.0-beta Cloud-G+ | Commercial Hardening | 商业级/规模化 | 云端持久化迁移边界已收口；后续真实账号、权限、对象存储 adapter、配额执行、审计写入、版权审批和部署观测仍需继续拆成小刀加固。 |
...
- v0.9.0-alpha 已整体收口，长篇共创形成 alpha 产品闭环；v0.9.1 provider/cost、v0.9.2 MasterSetting Lite、v0.9.3 Graph Memory Evaluation、v0.9.4 Advanced Runner Evaluation、v1.0-beta Scope-A、Schema-B、Matrix-C、Copyright-D、Quota-E、Deploy-F 与 Cloud-G 已整体收口，但仍需继续拆分真实账号、对象存储 adapter、审计写入等能力才接近商业级。
