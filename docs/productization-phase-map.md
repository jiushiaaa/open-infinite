...
| v0.9.1-v0.9.4 | Provider/Cost、MasterSetting Lite、Graph Memory Spike、Advanced Runner Spike | 真实使用压力增强 | v0.9.1 进行中：provider 摘要、usage 聚合、设置展示与手动估算已收口；后续按成本、长篇召回、设定管理、runner 复杂度触发增强，不提前引入重依赖。 |
...
- v0.9.0-alpha 已整体收口，长篇共创形成 alpha 产品闭环；v0.9.1 provider/cost 已启动，但仍需后续子刀和 v1.0-beta 才能接近商业级。
