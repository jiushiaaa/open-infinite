...
- 作者采纳后的章节 brief / 大纲差异 / Reviewer 修订 API 已有第一版；采纳、部分采纳、另开分支已能生成 `writing_plan` / `feed_forward` 并影响章节生成或后续沙盘入口；章节草稿生成、连续阅读稿、作者确认入卷、确认稿跨卷宗阅读链和卷宗阅读页已接入；仍需局部重写、更强 Reviewer、正文内锚点跳转和误会图谱。
