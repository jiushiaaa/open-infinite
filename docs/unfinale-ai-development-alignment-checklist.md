...
> 2026-06-04 状态：World Sandbox Loop v1-v8 已完成第一版可运行闭环；本清单后续用于判断“是否在加深活体小说体验”，而不是继续证明这些模块是否存在。
...
1. 干预可执行投放：在 S3 已能生成世界线快照后，让编译结果成为下一轮沙盘约束，而不是只停留在预检说明。
2. 主观记忆继续加深：在 S2 已有误会/盲区字段后，补长期召回、记忆压缩/遗忘、压抑记忆爆发、误会图谱和“角色不知道的正史事实”对照。
3. Agent 决策继续加深：下一轮可在显式 opt-in 下接真实模型 runner smoke，但默认仍保留 deterministic/mockable 基线。
4. 干预执行闭环：干预编译结果能成为下一轮沙盘约束，而不是停留在预检报告。
5. 世界状态持久化：世界线代偿、因果债和锚点变化能驱动后续轮次。
6. L5 觉醒反抗：角色能质疑读者、拒绝干预、假意服从或保护他人。
7. 多视角正文：从 brief 升级到可读章节和连续角色个人卷。
8. 作者采纳反哺：采纳结果能生成下一章 brief 并影响后续沙盘。
