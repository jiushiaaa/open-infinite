...
> 2026-06-08 整理结论：`memory.md` 已压回当前事实入口，完整流水历史继续放在 `project-changelog.md`。`docs/` 物理目录暂不大搬家，避免破坏历史链接；当前用本索引、`completed/README.md` 和各资料目录 README 做逻辑归类。
...
不要从 `project-changelog.md`、`completed/`、`后续增强清单.md` 或 `distribution-phase-plan.md` 直接捞下一刀。它们记录历史、支撑层或后置路径，不保证仍是当前路线。
...
| 分类 | 文档入口 | 当前口径 |
| --- | --- | --- |
| 已闭环历史底座 | `completed/README.md`、`project-changelog.md` | v0.7 到 v1.0-local、后续增强、真实 retrieval provider、Vector Retrieval Pipeline 均有历史记录和收口说明 |
...
| 仍需深入 | `../memory.md`、`living-novel-engine-iteration-plan.md` | 多轮策略规划、长期关系/势力博弈、真实长正文文风、跨章节误会/伏笔回收、更强真实语义 Reviewer 和整章风格润色 |
...
- 独立切片历史写到 `project-changelog.md` 末尾；不要改写旧日志。
