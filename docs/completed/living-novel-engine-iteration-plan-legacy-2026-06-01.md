...
v0.2.1   resume imported    已收口
    ↓
v0.2.2   精华固化           已收口 · 见 docs/completed/open-source-essence-absorption.md
    ↓
...
v1.0-beta Commercial Hardening Scope-A     商业化范围复核         已收口：详见 docs/completed/v1.0-beta-commercial-hardening-scope-a.md
...
v1.0-beta Worldline Selection Audit Hook-M  世界线选择审计钩子     已收口：详见 docs/completed/v1.0-beta-worldline-selection-audit-hook-m.md
v1.0-beta State Execution Audit Hook-N      状态执行审计钩子       已收口：详见 docs/completed/v1.0-beta-state-execution-audit-hook-n.md
v1.0-beta Commercial Status Overview-O      商业化状态总览         已收口：详见 docs/completed/v1.0-beta-commercial-status-overview-o.md
v1.0-beta Audit Log UI & Export-P           审计日志展示与导出     已收口：详见 docs/completed/v1.0-beta-audit-log-ui-export-p.md
v1.0-beta Settings Local Smoke Checklist-Q  设置页本地冒烟清单     已收口：详见 docs/completed/v1.0-beta-settings-local-smoke-checklist-q.md
v1.0-beta Release Preflight Checklist-R     发布前只读检查清单     已收口：详见 docs/completed/v1.0-beta-release-preflight-checklist-r.md
v1.0-beta Rights Approval Checklist-S       版权审批准备度清单     已收口：详见 docs/completed/v1.0-beta-rights-approval-checklist-s.md
v1.0-beta Deployment Observability Checklist-T 部署观测只读清单 已收口：详见 docs/completed/v1.0-beta-deployment-observability-checklist-t.md
v1.0-beta Auth Boundary Checklist-U         认证边界只读清单       已收口：详见 docs/completed/v1.0-beta-auth-boundary-checklist-u.md
...
| v0.9.1-v0.9.4 | Provider/Cost、MasterSetting、Graph Memory、Advanced Runner | 真实使用压力增强 | v0.9.1-v0.9.4 已整体收口；重依赖仍按触发式评估。 |
| v1.0-beta Scope-A | Commercial Hardening Scope | 商业化范围复核 | 已收口：只读 scope report，明确本地优先边界与延后项。 |
...
| v1.0-beta Audit Hook-M | Worldline Selection Audit Hook | 世界线选择审计钩子 | 已收口：世界线选择保存后追加 `worldline_selected` 审计事件。 |
| v1.0-beta Audit Hook-N | State Execution Audit Hook | 状态执行审计钩子 | 已收口：状态 overlay apply/rollback 写入本地审计日志。 |
| v1.0-beta Status-O | Commercial Status Overview | 商业化状态总览 | 已收口：设置页只读展示本地商业化状态域。 |
| v1.0-beta Audit UI-P | Audit Log UI & Export | 审计日志展示与导出 | 已收口：项目工作台展示审计时间线并支持 Markdown 导出。 |
| v1.0-beta Smoke-Q | Settings Local Smoke Checklist | 设置页本地冒烟清单 | 已收口：只读生成本地关键路径核对清单。 |
| v1.0-beta Preflight-R | Release Preflight Checklist | 发布前只读检查清单 | 已收口：只读聚合本地发布前证据，不执行真实发布。 |
| v1.0-beta Rights-S | Rights Approval Checklist | 版权审批准备度清单 | 已收口：项目工作台只读检查权利声明、授权确认、审计事件与公开发布保护。 |
| v1.0-beta Observe-T | Deployment Observability Checklist | 部署观测只读清单 | 已收口：设置抽屉只读聚合健康、冒烟、用量、job、审计、版权与发布前信号。 |
| v1.0-beta Auth-U | Auth Boundary Checklist | 认证边界只读清单 | 已收口：设置抽屉只读聚合账号空间、权限草案、ACL 缺口与部署观测边界。 |
| v1.0-beta ObjectStorage-V | Object Storage Boundary Checklist | 对象存储边界只读清单 | 已收口：设置抽屉只读聚合本地 artifact、资源映射、保留策略输入与 adapter 缺口。 |
| v1.0-beta QuotaEnforce-W | Quota Enforcement Boundary Checklist | 配额执行边界只读清单 | 已收口：设置抽屉只读聚合配额观测、usage metadata、内存 job、认证边界与部署观测。 |
| v1.0-beta BillingBoundary-X | Billing Adapter Boundary Checklist | 计费 adapter 边界只读清单 | 已收口：设置抽屉只读聚合 usage、成本估算、计费身份、支付 adapter、发票退款轨迹和写入边界。 |
| v1.0-local RunScripts | Local Run Scripts | 本地一键运行脚本 | 已收口：Windows/macOS 脚本可检查依赖、安装依赖并启动后端与前端。 |
...
**测试基线**：`cd engine && python -m pytest -q` → **713 passed**（截至 2026-06-01，v1.0-local Model Configuration UX + Local Run Scripts 收口后完整回归通过）；`cd engine/ui && pnpm run build` 通过。
...
未终章 应借鉴它作为“故事合约层 + 章节状态提交层”，但不直接依赖 Claude Code 插件运行时。
...
未终章 已在 v0.6.0-v0.6.3 建好自研 runner adapter / protocol / projection / trace UI。短期不接 MiroFish，是因为它会增加账号、服务部署、数据同步、环境适配和调试成本；LNE 当前更需要验证“叙事专用多 Agent”是否能提升角色计划、误解、隐瞒、延迟行动和关系传播。
...
2026-05-29 新增调研判断：`autonovel` 与 `AI_NovelGenerator` 不应进入未终章的核心架构。
...
| 未终章 | 读者 / 干预者 / 互动叙事用户 | 如何让小说世界继续运行，并允许读者改变命运 | 主线 |
...
| 维度 | eastworld | LNE 取舍 |
| --- | --- | --- |
| 产品场景 | 游戏 / 视觉小说 / 互动媒体 Agent 框架 | LNE 是可运行故事世界运行时 |
...
```text
docs/article/reports/eastworld-agent-interaction-triage.md
```
...
v0.9.0-alpha 已整体收口。它已把长篇上传、记忆、分支运行、审计、世界线选择和导出串成完整产品闭环：
...
> **v0.9.0-alpha Chapter Collection Export 已落地（2026-05-31）**：新增只读 `build_chapter_collection_export()` 与 `GET /api/runs/<run_id>/branches/<branch_id>/chapter-collection-export`，沿 `meta.parent_run_id` / `meta.parent_branch` 回溯父链，把父分支和当前续章按顺序合成 Markdown 合集；前端阅读区新增「导出合集」按钮。坏 id 400，缺当前章节 404，缺父链安全截断并返回 warning；不写 artifact、不改 `run_scene`、不导出上传原作全文或 holdout 私有正文。已验证：v0.9.0-alpha 测试扩充至 10 passed；完整后端 607 passed；前端 build 通过。

> **v0.9.0-alpha Export Share Guard 已落地（2026-05-31）**：单章导出与合集导出均新增 `share_guard`，Markdown 增加「版权与分享边界」；前端点击「导出章节」或「导出合集」时会先用中文确认版权责任。该 guard 只做本地导出前确认，不提供公开分享发布能力，不写 artifact，不改 runner。已验证：v0.9.0-alpha 测试保持 10 passed；完整后端 607 passed；前端 build 通过。

> **v0.9.0-alpha Creation Loop Completion Gate 已落地（2026-05-31）**：`creation_loop.checklist` 增加「确认版权边界」，并新增 additive `completion`，返回完成/总数、阻塞项、summary 与 `can_mark_alpha_complete`；前端「创作闭环」区显示闭环完成度。该判定只读、不写 artifact、不替代人工版本收口。已验证：v0.9.0-alpha 测试保持 10 passed；完整后端 607 passed；前端 build 通过。

> **v0.9.0-alpha Creation Loop Action Hints 已落地（2026-05-31）**：`creation_loop.completion.actions` 会根据阻塞项返回可执行/可跳转动作，包括生成世界线评审、设为下一章起点、查看回放与审计；前端完成度区展示快捷按钮，生成评审复用既有 `POST /api/runs/<run_id>/branches/<branch_id>/worldline-judgement`。该子刀只复用现有 API，不新增 runner 行为、不写额外 artifact。已验证：v0.9.0-alpha 测试扩充至 11 passed；完整后端 608 passed；前端 build 通过。

> **v0.9.0-alpha Creation Loop Readiness Evidence 已落地（2026-05-31）**：`creation_loop.completion.evidence` 会把每个清单项映射到 artifact、API 或页面 hash 依据；前端完成度区展示「判定依据」，帮助用户理解闭环为什么已完成、待处理或需复盘。该子刀只读、不写 artifact、不改 runner。已验证：v0.9.0-alpha 测试保持 11 passed；完整后端 608 passed；前端 build 通过。

> **v0.9.0-alpha Creation Loop Audit Quick Run 已落地（2026-05-31）**：当已选世界线缺少范围回放、且项目已有 baseline 与 holdout 章节时，`creation_loop.completion.actions` 返回 `run_replay_range`，包含复用现有 `POST /api/stories/<slug>/canon/replay-range` 的 payload；前端完成度区可直接运行范围回放并刷新工作台。该子刀不新增 API、不改 runner、不写额外 artifact（只触发现有 range replay artifact）。已验证：v0.9.0-alpha 测试扩充至 12 passed；完整后端 609 passed；前端 build 通过。

> **v0.9.0-alpha Creation Loop Alpha Ready State 已落地（2026-05-31）**：补齐 clean 导入审计、候选世界线、评审、Causal Diff、低风险范围回放、已选起点、章节导出和版权 guard 后，`creation_loop.completion.status=ready` 且 `can_mark_alpha_complete=true`；前端「创作闭环」标题状态显示「可收口」。该子刀仍不自动宣告版本完成，只提供可验证的 ready 状态。已验证：v0.9.0-alpha 测试扩充至 13 passed；完整后端 610 passed；前端 build 通过。
...
> **v0.9.0-alpha Creation Loop Stable Blocker IDs 已落地（2026-05-31）**：`creation_loop.closeout` 新增 additive `remaining_blocker_ids`，与 `completion.blocking_ids` 保持一致；原 `remaining_blockers` 中文 label 保留。该子刀用于 closeout API 自动验收稳定识别阻塞项，不改变 ready 判定、不执行动作。已验证：v0.9.0-alpha 测试保持 15 passed；完整后端 612 passed；前端 build 通过。

> **v0.9.0-alpha Replay Audit Action Requirements 已落地（2026-05-31）**：当 `replay_audit` 只能跳转而不能直接运行范围回放时，action 新增 additive `requirements`，说明缺少 `selected_worldline`、`baseline_run`、`canon_holdout`，或需要 `review_replay_risk`。该子刀只解释前置条件，不生成 baseline、不写 holdout、不执行范围回放。已验证：v0.9.0-alpha 测试保持 15 passed；完整后端 612 passed；前端 build 通过。

> **v0.9.0-alpha Requirements UI Display 已落地（2026-05-31）**：前端「创作闭环」完成度区会展示 action requirements 的「审计前置」，最多展示三项中文前置条件，帮助用户理解为什么当前只能跳转审计页或需要先补起点、baseline、holdout。该子刀只展示现有 API 字段，不新增按钮、不自动执行 action。已验证：完整后端 612 passed；前端 build 通过。

> **v0.9.0-alpha Builtin Holdout Blocked Requirement 已落地（2026-05-31）**：`replay_audit.requirements` 会读取 story source kind；builtin 样例缺少 holdout 时，`canon_holdout.status=blocked`，detail 提示需导入长篇项目后录入 holdout。imported 项目仍维持 `missing`，不改变 holdout 写入流程。已验证：v0.9.0-alpha 测试扩充至 16 passed；完整后端 613 passed；前端 build 通过。

> **v0.9.0-alpha Creation Loop Closeout CLI 已落地（2026-06-01）**：新增 `lne creation-loop-closeout <slug>`，本地复用项目工作台的只读 closeout 判定，输出 closeout 状态、阻塞 id、阻塞动作与下一步；`--json` 输出与 HTTP closeout 接口同构的 payload，`--require-ready` 在未 ready 时以退出码 1 失败，可作为导入项目 alpha 收口闸门。该 CLI 不写 artifact、不执行 action、不改变 runner。已验证：v0.9.0-alpha 测试扩充至 18 passed；完整后端 615 passed；前端 build 通过。

> **v0.9.0-alpha Creation Loop Closeout Record 已落地（2026-06-01）**：`lne creation-loop-closeout <slug>` 新增 `--write-report`，仅在 `closeout.can_close_alpha=true` 时向导入项目写入 additive `creation_loop_alpha_closeout.json`，记录 `story_slug`、`version`、`completion_status`、`actions`、`closeout` 与 `created_at`；未 ready 时不落盘，builtin 样例无项目目录时不可写。该记录用于导入项目通过 alpha 闸门后的本地收口证据，不执行 action、不替代用户选择、不改变 `run_scene` 默认行为。已验证：v0.9.0-alpha 测试扩充至 19 passed；完整后端 616 passed；前端 build 通过。

> **v0.9.0-alpha Low-risk Audit Closeout / Alpha Closure 已落地（2026-06-01）**：修正 closeout 判定，`risk_level=low` 的静态审计 info 提示不再阻断 `replay_audit`、`post_run_audit` 和 alpha ready；中高风险静态审计、范围回放中高风险、缺失实体、缺评审、缺选择、缺导出仍阻断。已用本地导入项目 `v090-alpha-proof` 跑通 `lne creation-loop-closeout v090-alpha-proof --require-ready --write-report --json`，结果 `ready_count=7/7`、`remaining_blocker_ids=[]`，并写入 `creation_loop_alpha_closeout.json`。完整收口说明见 `docs/completed/v0.9.0-alpha-long-creation-loop.md`。已验证：v0.9.0-alpha 测试扩充至 20 passed；完整后端 617 passed；前端 build 通过。
...
> **v0.9.0-alpha Resume Continue HTTP Job 已落地（2026-05-31）**：新增 `run_resume_continue()` service 与 `POST /api/jobs/resume-continue`，把推荐世界线从 CLI 提示推进到显式 opt-in 续写 job；前端「创作闭环」新增「生成下一章」，成功后跳到新 run 的 `linear` 分支。run/branch id 走安全校验，坏 id 400；生成沿用 `write_resume_output()`，不写 `intervention.json`，不改 `run_scene` 默认行为。已验证：新增/扩充 v0.9.0-alpha 与 job 测试；完整后端 602 passed；前端 build 通过。

> **v0.9.0-alpha Worldline Selection Persistence 已落地（2026-05-31）**：新增 `selected_worldline.json` additive 选择记录、`GET/POST /api/stories/<slug>/selected-worldline` 与前端「设为起点」按钮；`creation_loop` 会读回 `selected` 并标记候选 `is_selected`。坏 id 400、缺 story/run/branch 404，选择记录损坏时降级为空态；不驱动 runner、不改推荐排序、不改既有 run artifact。已验证：v0.9.0-alpha 测试扩充至 7 passed；完整后端 604 passed；前端 build 通过。

> **v0.9.0-alpha Post-run Audit Entry 已落地（2026-05-31）**：`creation_loop.post_run_audit` 围绕已选世界线聚合世界线评审、Causal Diff、静态一致性审计与章节范围回放摘要，返回风险等级、缺失实体、下一步动作和回放审计入口；前端「创作闭环」区展示选择后审计状态并可回到回放与审计。该入口只读，不写正史账本，不驱动 runner。已验证：v0.9.0-alpha 测试扩充至 8 passed；完整后端 605 passed；前端 build 通过。

> **整体收口**：v0.9.0-alpha Long Novel Creation Loop 已完成第一条长篇共创产品闭环。公开分享发布能力、provider/cost gateway、云端持久队列、对象存储、向量库、运行后审计写回正史账本和 overlay 自动驱动下一轮 runner 均未进入本版本；下一步进入 v0.9.1 Provider & Cost Gateway Lite。

> **v0.9.1 Provider Gateway Summary-A 已落地（2026-06-01）**：新增 `service.runtime_settings.get_provider_gateway_summary()` 与只读 `GET /api/settings/providers`。接口返回 `v0.9.1-provider-cost-lite` 摘要：LLM/Seedream provider 列表、单 provider 路由状态、mock/占位图降级策略、成本观测口径和 warning。该子刀不创建客户端、不打网络、不落盘、不返回明文 Key 或环境变量名；测试 `tests/test_runtime_settings_api.py` 扩充到 19 passed，完整后端 620 passed，前端 build 通过。

> **v0.9.1 Provider Usage Summary-B 已落地（2026-06-01）**：新增 `service.runtime_settings.get_provider_usage_summary(story_slug=None)` 与只读 `GET /api/settings/provider-usage`，扫描 `intervention_compilation.json` 与 `multi_agent_trace.json` 的 `generation_meta.usage`，汇总 totals、by_provider、records、缺失 usage meta 计数和空成本估算。`story_slug` 查询参数继续走 `safe_id`，非法值 400。该子刀不读 Key、不打网络、不写 artifact、不内置真实价格表；测试 `tests/test_runtime_settings_api.py` 扩充到 23 passed，完整后端 624 passed，前端 build 通过。
...
> **v0.9.1 Manual Price Estimate-D 已落地（2026-06-01）**：运行设置新增 `llm_input_cost_per_1k` / `llm_output_cost_per_1k`，负数或非数字 400，只写当前进程环境变量。Provider gateway 返回手动单价和 `price_table_status`；provider usage 在配置单价后按 prompt/completion 用量估算费用。设置抽屉新增「成本估算」输入区，用量区展示估算金额。该子刀不硬编码厂商价格、不联网查价、不写 artifact、不改模型调用链；设置测试扩充到 25 passed，完整后端目标基线 626 passed，前端 build 通过。

> **v0.9.1 Route Matrix-E 已落地（2026-06-01）**：`GET /api/settings/providers` 新增只读 `routes`，列出读者干预生成、主题创世、导入抽取、视觉资产生成四个入口的 provider、mode、runner 与 fallback；设置抽屉「模型与用量状态」新增路由矩阵行。该子刀只解释当前路由，不新增写入开关、不改默认 mock / Seedream / runner 调用链；设置测试保持 25 passed，前端 build 通过。

> **v0.9.1 Provider & Cost Gateway Lite 已整体收口（2026-06-01）**：五个子刀已覆盖多 provider 状态、Key 脱敏、模型路由解释、usage 汇总、手动成本估算、失败回退提示和设置页可视化；收口归档见 `docs/completed/v0.9.1-provider-cost-gateway-lite.md`。仍不做持久化 Key、完整商业网关、可写路由策略、厂商价格表、云端队列、对象存储或多租户配置中心。下一步进入 v0.9.2 MasterSetting Workspace Lite。

> **v0.9.2 MasterSetting Workspace Summary-A 已落地（2026-06-01）**：`get_project_workspace()` additive 返回 `master_setting_workspace`，只读聚合 `memory/master_setting.yaml`、`character_states/`、`timeline.yaml`、`plot_threads.yaml` 与 `chapters/`，提供 section count、世界规则、人物状态、时间线样例、伏笔样例、章节摘要样例、只读能力标记和下一步建议。损坏 MasterSetting 返回 `status=damaged` 与 warnings，不写 artifact、不改 runner；测试先红灯后实现，`tests/test_v088_long_project_workspace.py` 4 passed。

> **v0.9.2 MasterSetting Workspace Panel-B 已落地（2026-06-01）**：React 长篇项目工作台新增「设定工作台」只读面板，展示世界规则/限制/地点/势力、人物状态、时间线、伏笔线、章节摘要和后续建议；右侧项目资产面板新增设定状态。该子刀只读消费 `master_setting_workspace`，不写 `memory/` artifact、不做完整作者工作台、不改 runner。

> **v0.9.2 MasterSetting Workspace Edit-C 已落地（2026-06-01）**：新增 `POST /api/stories/<slug>/master-setting` 与 `update_master_setting()`，仅允许白名单编辑 `display_name`、`genre`、`world_rules`、`power_system_limits`、`forbidden_additions`；保存前备份 `memory/master_setting.yaml`，保存后写 `memory/master_setting_update_report.json`。缺故事 404，损坏/缺失 MasterSetting 409，非法 payload 400；不写人物/时间线/伏笔/章节摘要，不同步 `world.yaml`，不改 runner。

> **v0.9.2 MasterSetting Workspace Frontend-D 已落地（2026-06-01）**：React 长篇项目工作台「设定工作台」新增最小写控件，允许编辑作品名、题材、世界规则、力量限制、禁用设定，并通过 `POST /api/stories/<slug>/master-setting` 保存；保存后先本地更新面板并显示成功提示，再刷新项目工作台。后端 workspace ready 状态 additive 标记 `mode=lite_edit`、`can_edit=true`；missing/damaged 仍是只读空态。浏览器冒烟确认保存成功提示、新标题和规则内容同屏可见，console 无 warn/error；仍不编辑人物/时间线/伏笔/章节摘要，不同步 `world.yaml`，不改 runner。

> **v0.9.2 MasterSetting Workspace Lite 已整体收口（2026-06-01）**：四个子刀已覆盖项目级设定资产只读聚合、前端设定工作台、后端白名单轻编辑、前端最小写控件、保存备份与保存报告；收口归档见 `docs/completed/v0.9.2-master-setting-workspace-lite.md`。仍不做完整作者工作台、人物/时间线/伏笔/章节摘要编辑、`world.yaml` 同步、图数据库或云端协作。下一步进入 v0.9.3 Graph Memory Evaluation Spike 的触发条件复核。

> **v0.9.3 Graph Memory Evaluation Trigger-A 已落地（2026-06-01）**：新增 `evaluate_graph_memory_trigger()` 与 `GET /api/stories/<slug>/graph-memory-evaluation`，只读检查导入规模、canon ledger、entity aliases 与 consistency report，返回 `not_triggered` / `monitor` / `triggered`、触发原因、阈值与中文 next steps。非法 slug 返回 400，缺故事 404。该子刀不接 Zep、图数据库、GraphRAG、embedding、向量库或 reranker，不写 artifact，不替换现有 BM25 / ledger / aliases。已验证：新增 v0.9.3 测试 3 passed。

> **v0.9.3 Retrieval Probe-B / Graph Memory Evaluation Spike 已整体收口（2026-06-01）**：新增 `evaluate_retrieval_probes()` 与 `GET /api/stories/<slug>/retrieval-probes`，从 canon ledger 与 entity aliases 自动生成代表性查询样本，复用现有 `retrieve_context()` 检查期望 source、item 与实体命中，返回 `pass` / `weak` / `insufficient_samples`、命中率、失败样例和中文建议。非法 slug 返回 400，缺故事 404。收口归档见 `docs/completed/v0.9.3-graph-memory-evaluation-spike.md`。该版本仍不接 Zep、图数据库、GraphRAG、embedding、向量库或 reranker，不写新 run artifact，不改 runner。
...
> **v1.0-beta Commercial Hardening Scope-A 已收口（2026-06-01）**：新增 `get_commercial_hardening_scope()` 与 `GET /api/settings/commercial-hardening-scope`，只读整理账号与项目空间、权限模型、云端持久化、配额与成本护栏、审计日志、版权与分享边界、部署与观测七个商业化域，返回当前覆盖、缺口、本地优先下一步、平台化下一步、延后项和下一刀候选。收口归档见 `docs/completed/v1.0-beta-commercial-hardening-scope-a.md`。该版本不读/回显密钥、不创建客户端、不打网络、不落盘，不进入云端多租户、对象存储、持久队列或计费系统。下一刀进入 `v1.0-beta Commercial Audit Log Schema-B`。

> **v1.0-beta Commercial Audit Log Schema-B 已收口（2026-06-01）**：新增 `get_project_audit_log()` 与 `GET /api/stories/<slug>/audit-log`，定义本地 `memory/project_audit_log.jsonl` 审计事件 schema，并只读聚合导入检查、世界线选择、设定轻编辑、creation loop closeout 与现有 JSONL 行。非法 slug 返回 400，缺项目 404，坏 JSONL 行降级为 warning。收口归档见 `docs/completed/v1.0-beta-commercial-audit-log-schema-b.md`。该版本不写日志、不接账号/权限系统、不接云端不可篡改存储。下一刀进入 `v1.0-beta Permission Matrix Draft-C`。

> **v1.0-beta Permission Matrix Draft-C 已收口（2026-06-01）**：新增 `get_permission_matrix_draft()` 与 `GET /api/settings/permission-matrix`，只读返回 owner/editor/viewer 三角色、项目工作台、设定轻编辑、世界线选择、生成动作、审计日志、导出等资源的权限矩阵草案。报告明确 `enforcement.mode=not_enforced`，不接真实认证、不做服务端权限拦截。收口归档见 `docs/completed/v1.0-beta-permission-matrix-draft-c.md`。下一刀进入 `v1.0-beta Project Copyright Statement-D`。
...
> **v1.0-beta Project Retention Policy-J 已收口（2026-06-01）**：新增 `get_project_retention_policy()`、`write_project_retention_policy()` 与 `GET/POST /api/stories/<slug>/retention-policy`，本地写入 `memory/project_retention_policy.json`，覆盖项目、上传原文、生成产物、holdout、审计日志和 ingest 分片保留口径；写入后追加 `retention_policy_reviewed` 审计事件。当前不实际删除文件、不接对象存储、数据库、持久队列、真实账号或团队空间。收口归档见 `docs/completed/v1.0-beta-project-retention-policy-j.md`。后续继续拆分 v1.0-beta 商业化加固小刀。
...
> **v1.0-beta Worldline Selection Audit Hook-M 已收口（2026-06-01）**：`select_worldline()` 在成功写入 `selected_worldline.json` 后追加 `worldline_selected` 审计事件，导入项目的 `GET /api/stories/<slug>/audit-log` 可聚合该关键选择。builtin 样例仍保持 outputs 选择记录语义，审计追加冲突会降级跳过。收口归档见 `docs/completed/v1.0-beta-worldline-selection-audit-hook-m.md`。后续继续拆分 v1.0-beta 商业化加固小刀。

> **v1.0-beta State Execution Audit Hook-N 已收口（2026-06-01）**：`apply_runner_state_execution()` / `rollback_runner_state_execution()` 成功后分别追加 `state_execution_applied` / `state_execution_rolled_back` 审计事件，导入项目审计时间线可追踪状态 overlay 应用与回滚。builtin 样例保持既有状态执行语义，审计追加冲突会降级跳过。收口归档见 `docs/completed/v1.0-beta-state-execution-audit-hook-n.md`。后续继续拆分 v1.0-beta 商业化加固小刀。

> **v1.0-beta Commercial Status Overview-O 已收口（2026-06-01）**：新增 `get_commercial_status_overview()` 与 `GET /api/settings/commercial-status-overview`，设置抽屉新增「商业化状态总览」只读区，聚合商业化范围、provider/cost、配额观测、权限矩阵、账号/项目空间、云端持久化、本地部署、审计与版权边界。该刀不写 artifact、不执行认证/权限/云端/计费，只提供本地 beta 检查入口。收口归档见 `docs/completed/v1.0-beta-commercial-status-overview-o.md`。后续继续拆分 v1.0-beta 商业化加固小刀。

> **v1.0-beta Audit Log UI & Export-P 已收口（2026-06-01）**：新增 `export_project_audit_log()` 与 `GET /api/stories/<slug>/audit-log/export`，项目工作台新增「项目审计日志」只读区，展示事件数、来源产物、最近事件、warning 与下一步，并支持本地 Markdown 导出。导出不包含事件 metadata，不代表不可篡改审计证明；不接真实认证、权限、云端存储或计费。收口归档见 `docs/completed/v1.0-beta-audit-log-ui-export-p.md`。后续继续拆分 v1.0-beta 商业化加固小刀。

> **v1.0-beta Settings Local Smoke Checklist-Q 已收口（2026-06-01）**：新增 `get_settings_local_smoke_checklist()` 与 `GET /api/settings/local-smoke-checklist`，设置抽屉新增「本地冒烟清单」只读区，展示首页、故事列表、设置页、项目工作台和审计导出等本地核对路径。该刀只生成 checklist，不主动执行 HTTP 请求、不绑定端口、不落盘、不打外网。收口归档见 `docs/completed/v1.0-beta-settings-local-smoke-checklist-q.md`。后续继续拆分 v1.0-beta 商业化加固小刀。

> **v1.0-beta Release Preflight Checklist-R 已收口（2026-06-01）**：新增 `get_release_preflight_checklist()` 与 `GET /api/settings/release-preflight`，设置抽屉新增「发布前检查」只读区，聚合本地部署、冒烟、商业状态、权限草案、版权/保留/审计入口；未选项目时项目级项降级为“选择具体项目后核对”。该刀只读聚合本地证据，不执行真实发布、不主动打请求、不落盘、不打外网。收口归档见 `docs/completed/v1.0-beta-release-preflight-checklist-r.md`。Rights Approval Checklist-S 已继续收口版权审批准备度。

> **v1.0-beta Rights Approval Checklist-S 已收口（2026-06-01）**：新增 `get_rights_approval_checklist()` 与 `GET /api/stories/<slug>/rights-approval-checklist`，项目工作台「项目审计日志」区新增「版权审批检查」只读面板，聚合项目版权/来源声明、授权确认、local export 许可、`rights_reviewed` 审计事件和公开发布保护项。该刀只读检查准备度，不执行真实审批、不开放公开发布、不写 artifact、不打外网。收口归档见 `docs/completed/v1.0-beta-rights-approval-checklist-s.md`。Deployment Observability Checklist-T 已继续收口部署观测准备度。

> **v1.0-beta Deployment Observability Checklist-T 已收口（2026-06-01）**：新增 `get_deployment_observability_checklist()` 与 `GET /api/settings/deployment-observability`，设置抽屉新增「部署观测清单」只读区，聚合本地部署、冒烟、配额用量、内存 job、项目审计时间线、版权审批和发布前检查。该刀只读聚合本地证据，不 tail 日志、不写 artifact、不打外网、不接云端观测。收口归档见 `docs/completed/v1.0-beta-deployment-observability-checklist-t.md`。Auth Boundary Checklist-U 已继续收口认证边界。

> **v1.0-beta Auth Boundary Checklist-U 已收口（2026-06-01）**：新增 `get_auth_boundary_checklist()` 与 `GET /api/settings/auth-boundary`，设置抽屉新增「认证边界清单」只读区，聚合本地单人账号语义、owner/editor/viewer 权限草案、请求级 ACL 缺口、项目空间映射和部署观测边界。该刀只读解释认证边界，不创建真实用户、不接认证 provider、不执行 ACL 拦截、不写 artifact、不打外网。收口归档见 `docs/completed/v1.0-beta-auth-boundary-checklist-u.md`。Object Storage Boundary Checklist-V 已继续收口对象存储边界。

> **v1.0-beta Object Storage Boundary Checklist-V 已收口（2026-06-01）**：新增 `get_object_storage_boundary_checklist()` 与 `GET /api/settings/object-storage-boundary`，设置抽屉新增「对象存储边界」只读区，聚合本地 artifact 盘点、资源映射、原文/holdout 私有隔离、项目保留策略输入、身份绑定和本地部署护栏。该刀只读解释对象存储 adapter 前置边界，不创建 bucket、不生成签名 URL、不上传文件、不写 artifact、不打外网。收口归档见 `docs/completed/v1.0-beta-object-storage-boundary-checklist-v.md`。Quota Enforcement Boundary Checklist-W 已继续收口配额执行边界。

> **v1.0-beta Quota Enforcement Boundary Checklist-W 已收口（2026-06-01）**：新增 `get_quota_enforcement_boundary_checklist()` 与 `GET /api/settings/quota-enforcement-boundary`，设置抽屉新增「配额执行边界」只读区，聚合配额观测、usage metadata、内存 job、认证边界和部署观测。该刀只读解释硬配额 guardrail 与账单 adapter 的前置边界，不拦截生成请求、不写 quota state、不接真实账单。收口归档见 `docs/completed/v1.0-beta-quota-enforcement-boundary-checklist-w.md`。Billing Adapter Boundary Checklist-X 已继续收口计费 adapter 边界。

> **v1.0-beta Billing Adapter Boundary Checklist-X 已收口（2026-06-01）**：新增 `get_billing_adapter_boundary_checklist()` 与 `GET /api/settings/billing-adapter-boundary`，设置抽屉新增「计费边界」只读区，聚合 provider usage、成本估算、配额执行边界、认证边界、计费身份、支付 provider adapter、发票退款轨迹和计费写入边界。该刀只读解释真实计费 adapter 前置条件，不创建 customer/subscription/checkout/webhook，不写余额、账单、套餐、欠费、发票、退款或支付状态，不打外网、不读取明文密钥。收口归档见 `docs/completed/v1.0-beta-billing-adapter-boundary-checklist-x.md`。后续继续拆分真实硬配额 guardrail 前置清单、webhook/idempotency 边界或认证执行前置清单。
...
| P10-A | v1.0-beta Commercial Hardening Scope | 账号/项目空间、权限、云端持久化、配额、审计日志、版权提示、部署与观测范围复核 | 已收口 |
...
| P10-M | v1.0-beta Worldline Selection Audit Hook | 世界线选择写入接入本地审计日志 | 已收口 |
| P10-N | v1.0-beta State Execution Audit Hook | 状态 overlay apply/rollback 接入本地审计日志 | 已收口 |
| P10-O | v1.0-beta Commercial Status Overview | 设置页商业化状态总览 | 已收口 |
| P10-P | v1.0-beta Audit Log UI & Export | 项目工作台审计时间线展示与 Markdown 导出 | 已收口 |
| P10-Q | v1.0-beta Settings Local Smoke Checklist | 设置页本地冒烟清单 | 已收口 |
| P10-R | v1.0-beta Release Preflight Checklist | 发布前只读检查清单 | 已收口 |
| P10-S | v1.0-beta Rights Approval Checklist | 项目版权审批准备度只读检查 | 已收口 |
| P10-T | v1.0-beta Deployment Observability Checklist | 设置页部署观测只读清单 | 已收口 |
| P10-U | v1.0-beta Auth Boundary Checklist | 设置页认证边界只读清单 | 已收口 |
...
| D0 | Distribution Phase Plan | 本地 clone、GitHub Release 安装包、服务器在线体验三条使用路径排期 | 已排期 |
| D1 | Local Run Scripts | Windows/macOS 本地一键运行脚本 | 已收口；安装包和内置 runtime 后置 |
...
- [x] `docs/completed/open-source-essence-absorption.md`
...
先写专题研读报告，再按需吸收。仍在分析中的论文/项目报告放 `docs/article/reports/`；已完成吸收决策的版本记录归档到 `docs/completed/`：
...
v0.7.1 Intervention Compiler（自由输入转抽象干预 + 动态分支轴，已完成）
  ...
  -> v1.0-beta Commercial Hardening Scope-A（已收口：只读范围报告）
  ...
  -> v1.0-beta Worldline Selection Audit Hook-M（已收口：世界线选择审计钩子）
  -> v1.0-beta State Execution Audit Hook-N（已收口：状态执行审计钩子）
  -> v1.0-beta Commercial Status Overview-O（已收口：商业化状态总览）
  -> v1.0-beta Audit Log UI & Export-P（已收口：审计日志展示与导出）
  -> v1.0-beta Settings Local Smoke Checklist-Q（已收口：设置页本地冒烟清单）
  -> v1.0-beta Release Preflight Checklist-R（已收口：发布前只读检查清单）
  -> v1.0-beta Rights Approval Checklist-S（已收口：版权审批准备度清单）
  -> v1.0-beta Deployment Observability Checklist-T（已收口：部署观测只读清单）
  -> v1.0-beta Auth Boundary Checklist-U（已收口：认证边界只读清单）
  ...
  -> v1.0-local Local Run Scripts（已收口：Windows/macOS 本地一键运行脚本）
  -> 暂停继续商业化/计费实现，后续等本地产品稳定后进入安装包/服务器发行路径
```

WenShape 解决“长篇上下文怎么不崩”，webnovel-writer 解决“故事合约和网文味”，MiroFish 解决“角色群体怎么自己动起来”，eastworld 解决“互动媒体 Agent 如何做动作、情绪查询和玩家护栏”的参考问题；四篇论文分别补上“用户涌现、意图调度、叙事质量、动作落地”的理论底座。未终章 自己要牢牢抓住的，是它们都没有真正覆盖的核心：

> 读者干预、世界线分支、角色反抗、故事世界运行时。
