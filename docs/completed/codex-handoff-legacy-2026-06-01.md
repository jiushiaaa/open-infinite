...
- v1.0-beta Local Deployment Readiness-F
- v1.0-beta Cloud Persistence Boundary-G
...
- v1.0-beta Permission Matrix Draft-C：新增只读 `GET /api/settings/permission-matrix`，返回 owner/editor/viewer 三角色与项目工作台、设定编辑、世界线选择、生成动作、审计日志、导出等资源权限；明确 `enforcement.mode=not_enforced`
- v1.0-beta Project Copyright Statement-D：新增 `GET/POST /api/stories/<slug>/copyright-statement`，本地写入 `memory/project_copyright_statement.json`；导出 `share_guard.rights_basis` 引用项目级声明，继续不开放公开发布入口
- v1.0-beta Quota & Observability Lite-E：新增只读 `GET /api/settings/quota-observability`，汇总 provider usage、内存 job 状态、软配额口径和观测缺口；`quota_policy.mode=not_enforced`
- v1.0-beta Local Deployment Readiness-F：新增只读 `GET /api/settings/deployment-readiness`，汇总本地 HTTP 入口、静态资源、脱敏环境、数据目录、API 冒烟计划和运行/验证步骤；`external_services_required=false`
- v1.0-beta Cloud Persistence Boundary-G：新增只读 `GET /api/settings/cloud-persistence-boundary`，映射本地 artifact 到未来平台资源、保留规则和迁移边界；`migration.mode=not_started`，不接对象存储、数据库或持久队列
...
- 完成后同步 memory.md 当前状态；如需追加历史记录，同步 docs/project-changelog.md；必要时同步迭代计划/README/UI spec/codex-handoff
```

## 当前项目状态

未终章 是 `D:\AI\open-infinite\engine` 下的未终章运行时。核心闭环：
...
| v0.9.1-v1.0-beta | 增强与商业化 | provider/cost、MasterSetting、图记忆/advanced runner 评估、商业化范围复核、本地审计 schema、权限矩阵草案、项目级版权声明、本地配额/观测、部署就绪、云端持久化边界、账号/项目空间边界、审计追加策略、项目保留策略、关键写操作审计钩子、设置页商业化状态总览、本地冒烟、发布前检查、版权审批准备度和部署观测清单已收口；后续继续本地优先商业化加固 |
...
## v1.0-beta Permission Matrix Draft-C 收口摘要

- 新增 `service.commercial_permissions.get_permission_matrix_draft()`。
- 新增 `GET /api/settings/permission-matrix`。
- 报告定义 owner/editor/viewer 三角色，并列出项目工作台、设定轻编辑、世界线选择、生成动作、审计日志、导出等资源的当前 endpoint 与角色权限。
- `enforcement.mode=not_enforced`，明确本刀只是权限设计草案，不接认证、不执行权限拦截。
- 收口归档见 `docs/completed/v1.0-beta-permission-matrix-draft-c.md`。
- 边界：不接真实账号、团队空间、认证 provider 或请求上下文；不新增权限拦截，不改变现有 API 行为。
- 验证：`tests/test_v100_permission_matrix.py` 2 passed；完整后端基线提升到 652 passed。

## v1.0-beta Project Copyright Statement-D 收口摘要

- 新增 `service.copyright_statement.get_project_copyright_statement()` / `write_project_copyright_statement()`。
- 新增 `GET/POST /api/stories/<slug>/copyright-statement`；slug 走安全校验，非法 slug 400，缺项目 404，内置样例写入 409。
- 导入项目声明写入 `memory/project_copyright_statement.json`；损坏声明返回 `status=damaged` warning，不让接口 500。
- 单章导出与合集导出的 `share_guard` 新增 additive `rights_basis`，Markdown 版权段写入项目声明状态、来源标题、权利状态和声明用途。
- 收口归档见 `docs/completed/v1.0-beta-project-copyright-statement-d.md`。
- 边界：不开放公开分享/发布/商用入口，不接平台版权审核、风控、云端存储或法律判断，不改 runner 或既有 artifact 契约。
- 验证：`tests/test_v100_copyright_statement.py` 7 passed；导出回归 `tests/test_v090_long_creation_loop.py tests/test_v100_copyright_statement.py` 27 passed；完整后端基线提升到 659 passed。

## v1.0-beta Quota & Observability Lite-E 收口摘要

- 新增 `service.quota_observability.get_quota_observability_lite()`。
- 新增只读 `GET /api/settings/quota-observability`；可选 `story_slug` 过滤，非法 story slug 400。
- 报告复用 provider usage 汇总，返回 token totals、缺失 usage 计数、provider 汇总、当前内存 job 状态、软配额口径和观测缺口。
- `JobStore` 新增 additive `snapshot()` 与 `max_jobs`，仅用于观测当前进程内 job 状态。
- 收口归档见 `docs/completed/v1.0-beta-quota-observability-lite-e.md`。
- 边界：不执行硬配额、不阻断生成请求，不接真实计费、云端监控、日志平台、告警系统或多租户账号，不写 artifact。
- 验证：`tests/test_v100_quota_observability.py` 2 passed；运行设置回归 `tests/test_runtime_settings_api.py tests/test_v100_quota_observability.py` 29 passed；完整后端基线提升到 661 passed。

## v1.0-beta Local Deployment Readiness-F 收口摘要

- 新增 `service.deployment_readiness.get_local_deployment_readiness()`。
- 新增只读 `GET /api/settings/deployment-readiness`。
- 报告汇总本地后端 HTTP 入口、前端静态资源、运行环境脱敏、本地数据目录、API 冒烟计划、运行步骤和验证步骤。
- `readiness.external_services_required=false`；静态资源或数据目录缺失时返回 `attention`，不抛 500。
- 收口归档见 `docs/completed/v1.0-beta-local-deployment-readiness-f.md`。
- 边界：不接云端托管、对象存储、多用户账号、商业计费、外部监控、告警系统或自动部署脚本；不打网络、不落盘、不改 `run_scene`。
- 验证：`tests/test_v100_deployment_readiness.py` 2 passed；运行设置/配额/部署邻近回归 31 passed；完整后端基线提升到 663 passed。

## v1.0-beta Cloud Persistence Boundary-G 收口摘要

- 新增 `service.cloud_persistence_boundary.get_cloud_persistence_boundary()`。
- 新增只读 `GET /api/settings/cloud-persistence-boundary`。
- 报告映射 `projects/`、`outputs/`、`_ingest_sessions/` 相关 artifact 到未来平台资源，并补充项目删除、分片过期、审计追加和 holdout 私有隔离保留规则。
- `migration.mode=not_started`、`external_services_required=false`；本地 inventory 不返回绝对路径，不暴露密钥名或密钥值。
- 收口归档见 `docs/completed/v1.0-beta-cloud-persistence-boundary-g.md`。
- 边界：不接对象存储、数据库、持久队列、跨设备恢复、云端账号或团队空间；不上传文件、不迁移数据、不写 artifact、不改 `run_scene`。
- 验证：`tests/test_v100_cloud_persistence_boundary.py` 2 passed；商业化相邻回归 33 passed；完整后端基线提升到 665 passed。

## v1.0-beta Account Project Space Boundary-H 收口摘要

- 新增 `service.account_project_space.get_account_project_space_boundary()`。
- 新增只读 `GET /api/settings/account-project-space-boundary`。
- 报告定义本地单用户账号语义、导入项目/内置样例/run/世界线选择空间清单、未来 metadata 字段和迁移边界。
- `account_model.mode=local_single_operator`、`enforcement.mode=not_enforced`；不返回绝对路径，不暴露密钥名或密钥值。
- 收口归档见 `docs/completed/v1.0-beta-account-project-space-boundary-h.md`。
- 边界：不接真实账号、团队空间、认证 provider、成员邀请、跨设备同步或请求级 ACL；不新增权限拦截、不写 artifact、不迁移项目、不改 `run_scene`。
- 验证：`tests/test_v100_account_project_space_boundary.py` 2 passed；商业化相邻回归 42 passed；完整后端基线提升到 667 passed。

## v1.0-beta Audit Log Append Policy-I 收口摘要

- 新增 `service.commercial_audit_log.append_project_audit_log_event()`。
- 新增 `POST /api/stories/<slug>/audit-log/events`。
- 白名单追加本地 `memory/project_audit_log.jsonl`，坏 payload 400、缺项目 404、内置样例只读 409；`metadata` 会丢弃疑似密钥字段或密钥值。
- `GET /api/stories/<slug>/audit-log` 继续聚合追加后的 JSONL 行；权限矩阵草案同步将审计日志资源更新为 `read + append`。
- 收口归档见 `docs/completed/v1.0-beta-audit-log-append-policy-i.md`。
- 边界：不接真实账号、团队空间、认证 provider、请求级 ACL、云端不可篡改审计存储、对象存储、数据库或队列；不自动为所有写操作补审计，不改 `run_scene`。
- 验证：`tests/test_v100_audit_log_append_policy.py` 6 passed；商业化相邻回归 44 passed；权限同步回归 13 passed；完整后端基线提升到 673 passed。

## v1.0-beta Project Retention Policy-J 收口摘要

- 新增 `service.project_retention_policy.get_project_retention_policy()`。
- 新增 `service.project_retention_policy.write_project_retention_policy()`。
- 新增 `GET/POST /api/stories/<slug>/retention-policy`。
- 本地写入 `memory/project_retention_policy.json`，覆盖项目、上传原文、生成产物、holdout、审计日志和 ingest 分片的保留口径；写入后追加 `retention_policy_reviewed` 审计事件。
- 收口归档见 `docs/completed/v1.0-beta-project-retention-policy-j.md`。
...
## v1.0-beta MasterSetting Audit Hook-L 收口摘要

- `update_master_setting()` 成功写入 `memory/master_setting.yaml` 与 `memory/master_setting_update_report.json` 后追加 `master_setting_updated` 审计事件。
- 审计事件写入 `memory/project_audit_log.jsonl`，并带上 `artifact_path=memory/master_setting.yaml`、`report_path` 与 `changed`。
- 审计追加白名单新增 `master_setting_updated`。
- `GET /api/stories/<slug>/audit-log` 继续聚合该关键写操作。
- 收口归档见 `docs/completed/v1.0-beta-master-setting-audit-hook-l.md`。
...
## v1.0-beta State Execution Audit Hook-N 收口摘要
...
- 收口归档见 `docs/completed/v1.0-beta-state-execution-audit-hook-n.md`。
- 边界：不改变状态执行 eligibility、候选筛选、overlay 写入/回滚规则；不接真实账号、团队空间、认证 provider、请求级 ACL、云端不可篡改审计存储、对象存储、数据库或队列；不改 `run_scene`。
- 验证：`tests/test_v0810_runner_state_execution.py` 新增状态执行审计断言；邻近回归 20 passed；完整后端基线提升到 683 passed。

## v1.0-beta Commercial Status Overview-O 收口摘要
...
- 收口归档见 `docs/completed/v1.0-beta-commercial-status-overview-o.md`。
- 边界：不执行真实认证、权限拦截、云端迁移、对象存储或计费；不改 `run_scene`。
- 验证：`tests/test_v100_commercial_status_overview.py` 新增 service/HTTP 测试；邻近回归 35 passed；前端 build 通过；Vite + 后端 HTTP smoke 通过；完整后端基线提升到 685 passed。

## v1.0-beta Audit Log UI & Export-P 收口摘要
...
## v1.0-beta Release Preflight Checklist-R 收口摘要
...
## v1.0-beta Deployment Observability Checklist-T 收口摘要
...
- 清单聚合本地部署、本地冒烟、配额用量、内存 job、项目审计时间线、版权审批和发布前检查。
- 收口归档见 `docs/completed/v1.0-beta-deployment-observability-checklist-t.md`。
- 边界：不 tail 日志、不写 artifact、不打外网、不接云端观测、对象存储、持久队列、真实认证或计费；不改 `run_scene`。
- 验证：`tests/test_v100_deployment_observability_checklist.py` 新增 service/HTTP 测试；部署/发布/配额邻近回归 10 passed；前端 build 通过；完整后端基线提升到 700 passed。

## v1.0-beta Auth Boundary Checklist-U 收口摘要

- 新增 `get_auth_boundary_checklist()` 与 `GET /api/settings/auth-boundary`。
- 设置抽屉新增「认证边界清单」只读区，展示已具备/需留意数量、认证执行状态、前 5 条检查项和下一步。
- 清单聚合本地单人账号语义、owner/editor/viewer 权限草案、请求级 ACL 缺口、项目空间映射和部署观测边界。
- 收口归档见 `docs/completed/v1.0-beta-auth-boundary-checklist-u.md`。
...
## v1.0-beta Quota Enforcement Boundary Checklist-W 收口摘要

- 新增 `get_quota_enforcement_boundary_checklist()` 与 `GET /api/settings/quota-enforcement-boundary`。
- 设置抽屉新增「配额执行边界」只读区，展示已具备/需留意数量、硬配额状态、前 6 条检查项和下一步。
- 清单聚合配额观测、usage metadata、内存 job、认证边界和部署观测，明确硬配额 guardrail 与账单 adapter 仍未接入。
- 收口归档见 `docs/completed/v1.0-beta-quota-enforcement-boundary-checklist-w.md`。
- 边界：不拦截生成/导入/导出/视觉生成请求，不写 quota state，不接真实计费、账单、余额、支付、云端监控或告警；不改 `run_scene`。
- 验证：`tests/test_v100_quota_enforcement_boundary_checklist.py` 新增 service/HTTP 测试；配额/认证/部署观测邻近回归 9 passed；前端 build 通过；完整后端基线提升到 706 passed。

## v1.0-beta Billing Adapter Boundary Checklist-X 收口摘要
...
- 收口归档见 `docs/completed/v1.0-beta-billing-adapter-boundary-checklist-x.md`。
- 边界：不创建 customer/subscription/checkout/webhook，不写余额、账单、套餐、欠费、发票、退款或支付状态，不打外网、不读取明文密钥；不改 `run_scene`。
- 验证：`tests/test_v100_billing_adapter_boundary_checklist.py` 新增 service/HTTP 测试；计费/配额/认证/部署观测邻近回归 35 passed；前端 build 通过；完整后端基线提升到 708 passed。
...
| v1.0-beta Matrix-C | Permission Matrix Draft | owner/editor/viewer 权限矩阵草案 | 已收口 |
| v1.0-beta Copyright-D | Project Copyright Statement | 项目级版权/来源声明 schema | 已收口 |
| v1.0-beta Quota-E | Quota & Observability Lite | 本地配额、用量、job 状态与观测摘要 | 已收口 |
| v1.0-beta Deploy-F | Local Deployment Readiness | 健康检查、环境脱敏、静态资源/API 冒烟和运行步骤 | 已收口 |
| v1.0-beta Cloud-G | Cloud Persistence Boundary | 本地 artifact 到未来平台资源的映射、保留规则和迁移边界 | 已收口 |
...
| v1.0-beta Audit Hook-N | State Execution Audit Hook | 状态 overlay apply/rollback 接入本地审计日志 | 已收口 |
| v1.0-beta Status-O | Commercial Status Overview | 设置页商业化状态总览 | 已收口 |
| v1.0-beta Audit UI-P | Audit Log UI & Export | 项目工作台审计时间线展示与 Markdown 导出 | 已收口 |
| v1.0-beta Smoke-Q | Settings Local Smoke Checklist | 设置页本地冒烟清单 | 已收口 |
| v1.0-beta Preflight-R | Release Preflight Checklist | 发布前只读检查清单 | 已收口 |
...
- 代码清洁：`git diff --check`
- 文档：更新 `memory.md` 当前状态；如需追加历史记录，写入 `docs/project-changelog.md`
