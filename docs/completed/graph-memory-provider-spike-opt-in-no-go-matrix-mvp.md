# Graph Memory Provider Spike Opt-in No-go Matrix MVP 收口说明

日期：2026-06-02

## 目标

把 `Graph Memory Provider Spike Opt-in Evidence Snapshot` 继续收束为只读 no-go 分类矩阵。矩阵只回答“哪些阻塞类别仍禁止真实 provider 配置”，不保存签名，也不创建真实 provider 配置。
...
新增 `living_novel_engine.service.get_graph_memory_provider_spike_opt_in_no_go_matrix(slug, projects_dir=None, now=None)`。

报告字段包括：

- `summary`：来源 opt-in snapshot 状态、provider 数、矩阵行数、矩阵格数、阻塞格数和各类阻塞计数。
- `no_go_matrix`：`no_go_matrix_ready` / `needs_more_evidence` / `blocked` / `deferred`。
- `matrix_rows`：按 provider 输出签收、opt-in 材料、回滚材料、真实 provider 配置、外部账号或 Key 五类 no-go cell。
- `decision`：只允许人工复核 no-go 分类矩阵，不允许自动创建真实配置。
...
```text
GET /api/stories/<slug>/graph-memory-provider-spike-opt-in-no-go-matrix
```
...
项目工作台新增 `Graph 记忆 Provider Spike Opt-in No-go 矩阵` 面板，展示：

- 矩阵状态、阻塞格数、签收阻塞数和真实配置禁止状态。
- provider no-go rows、分类 cells、阻塞原因和 decision。
...
- endpoint：`/api/stories/{slug}/graph-memory-provider-spike-opt-in-no-go-matrix`
- typed client method：`getGraphMemoryProviderSpikeOptInNoGoMatrix`
- response type：`GraphMemoryProviderSpikeOptInNoGoMatrixReport`

当前契约计数更新为：

- `endpoint_count=56`
- `openapi_path_count=55`
- `typed_client_method_count=55`
...
python -m pytest tests/test_graph_memory_provider_spike_opt_in_no_go_matrix.py tests/test_api_contract.py -q
```
...
- `GET /api/stories/graph-fixture-pack-large/graph-memory-provider-spike-opt-in-no-go-matrix` 返回 `ready_for_opt_in_no_go_matrix`、`no_go_matrix_ready`、`matrix_row_count=2`、`matrix_cell_count=10`、`blocked_cell_count=6`、`writes_artifacts=false`、`external_services_required=false`、`real_provider_config_allowed=false`、`plaintext_key_returned=false`。
- `lne memory graph-opt-in-no-go-matrix graph-fixture-pack-large --json` 返回同一 ready 路径摘要。
- 前端 build 产物包含 `Graph 记忆 Provider Spike Opt-in No-go 矩阵`、`矩阵就绪`、`签收阻塞`、`仍禁止真实配置`、`真实配置` 等文案。
...
结果：后端 `843 passed`；`git diff --check` 通过，只有既有 CRLF 提示。

## 边界复核

- 路径安全：HTTP `slug` 经 `safe_id` 校验。
- 只读性：service 只消费 opt-in evidence snapshot，不写任何项目文件。
...
- 基于 opt-in no-go matrix 输出只读人工操作 checklist。
- 聚合暂停/升级判断、证据核对顺序、人工确认项和继续暂缓条件。
