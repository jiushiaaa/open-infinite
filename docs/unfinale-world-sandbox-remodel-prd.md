...
- 世界沙盘页：运行台已前置到首屏，默认先写大事件并启动一轮；单轮结果后先显示“本轮已发生”承接台，把事件、角色行动数、主观记忆数、因果债、锚点/资源/秘密变化和关键角色整理成结果总览，并提供读成正文、看世界线、生成多视角或再推一轮。
...
- 世界沙盘页运行导览、首屏运行台与结果承接台：运行台已前置到沙盘页首屏，桌面在 hero 右侧、390px 移动端排在标题说明后和导览前；默认路径是先写大事件并立即启动一轮推演，读者干预、投放对象、投放方式和真实模型建议仍保留为可选项；导览层继续说明“投放事件 -> 观察角色 -> 进入阅读”，出结果后先用“本轮已发生”承接台解释世界如何把事件消化成行动、记忆、因果债和下一章，再引导到卷宗阅读、世界线档案、多视角卷或继续运行。
