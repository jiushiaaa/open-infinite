...
- 世界沙盘页：运行台已前置到首屏，默认先写大事件并启动一轮；单轮结果后先显示“本轮已发生”承接台，把事件、角色行动数、主观记忆数、因果债、锚点/资源/秘密变化和关键角色整理成结果总览，并提供读成正文、看世界线、生成多视角或再推一轮；若本轮启用真实模型 advisory 且写出策略互动，结果承接后会先出现“策略棋盘”，把谁在算计谁、私下目的、筹码、误判、风险和预期世界影响提前整理出来，再进入角色行动链。
...
- 世界沙盘页运行导览、首屏运行台、结果承接台与策略棋盘：运行台已前置到沙盘页首屏，桌面在 hero 右侧、390px 移动端排在标题说明后和导览前；默认路径是先写大事件并立即启动一轮推演，读者干预、投放对象、投放方式和真实模型建议仍保留为可选项；导览层继续说明“投放事件 -> 观察角色 -> 进入阅读”，出结果后先用“本轮已发生”承接台解释世界如何把事件消化成行动、记忆、因果债和下一章，再引导到卷宗阅读、世界线档案、多视角卷或继续运行；真实模型策略互动存在时，策略棋盘会在结果总览和角色行动链之间展示角色算计路线、私下目的、误判和世界影响。
