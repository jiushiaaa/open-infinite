...
| 测试基线 | `cd engine && python -m pytest -q` -> `925 passed`；`cd engine/ui && pnpm run build` 通过 |
| 官方下一步 | **真实模型决策 + 长正文质量 + 作者确认**：S4 后半与 S5/S6/S7/S8/S9 已形成一条可继续运行的产品链路，新增 `worldline_state.json`、`consequence_state` 六域代偿、自演任务状态、检查点回放、多视角正文、下一章 brief 和正式下一章草稿；世界线/检查点独立页已补第一版 dossier 与回放入口；下一步优先加强真实 LLM 多 Agent 决策、长正文质量、作者可编辑确认和跨卷宗阅读；不默认回到 provider/Graph/检索评测主线 |
...
8. 作者采纳台：已可把沙盘涌现剧情采纳、部分采纳、另开分支或导出 brief，并写入 `author_adoption_ledger.jsonl`；采纳 run 可继续生成 `next_chapter_brief.json`、`next_chapter_draft.json` 和 `next_chapter_draft.md`。
```
