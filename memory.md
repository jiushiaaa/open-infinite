...
| 项 | 值 |
|----|-----|
| **测试基线** | 后端 `704 passed`（2026-06-01，v1.0-beta Object Storage Boundary Checklist-V 收口后完整回归通过）；前端 `engine/ui` typecheck + vite build 通过 |
...
| **刚收口** | v1.0-beta Object Storage Boundary Checklist-V：设置抽屉只读展示对象存储边界清单。 |

---

## 3.1 v0.8 收束期版本编排

当前已完成 **v0.9.3 Graph Memory Evaluation Spike** 的触发式复核：Trigger-A 给出规模/账本/别名/审计触发报告，Retrieval Probe-B 用现有 BM25、canon ledger 与 entity aliases 复跑代表性查询并收集失败样例。当前结论是不默认引入 Zep、图数据库或 GraphRAG；只有真实长篇 query 失败并证明文件型检索修复后仍不足时，才进入重依赖 spike。
...
| v1.0-beta CopyrightAudit-K | Copyright Audit Hook | 版权声明审计接入 | 已收口，见 `docs/completed/v1.0-beta-copyright-audit-hook-k.md` |
| v1.0-beta MasterSettingAudit-L | MasterSetting Audit Hook | 设定轻编辑审计接入 | 已收口，见 `docs/completed/v1.0-beta-master-setting-audit-hook-l.md` |
...
| v1.0-beta SettingsSmoke-Q | Settings Local Smoke Checklist | 设置页本地冒烟清单 | 已收口，见 `docs/completed/v1.0-beta-settings-local-smoke-checklist-q.md` |
| v1.0-beta ReleasePreflight-R | Release Preflight Checklist | 发布前只读检查清单 | 已收口，见 `docs/completed/v1.0-beta-release-preflight-checklist-r.md` |
| v1.0-beta RightsApproval-S | Rights Approval Checklist | 项目版权审批准备度清单 | 已收口，见 `docs/completed/v1.0-beta-rights-approval-checklist-s.md` |
| v1.0-beta DeployObserve-T | Deployment Observability Checklist | 部署观测只读清单 | 已收口，见 `docs/completed/v1.0-beta-deployment-observability-checklist-t.md` |
| v1.0-beta AuthBoundary-U | Auth Boundary Checklist | 认证边界只读清单 | 已收口，见 `docs/completed/v1.0-beta-auth-boundary-checklist-u.md` |
...
✅ v1.0-beta Project Retention Policy-J（已收口，见 docs/completed/v1.0-beta-project-retention-policy-j.md）
✅ v1.0-beta Copyright Audit Hook-K（已收口，见 docs/completed/v1.0-beta-copyright-audit-hook-k.md）
✅ v1.0-beta MasterSetting Audit Hook-L（已收口，见 docs/completed/v1.0-beta-master-setting-audit-hook-l.md）
...
✅ v1.0-beta Settings Local Smoke Checklist-Q（已收口，见 docs/completed/v1.0-beta-settings-local-smoke-checklist-q.md）
...
- [x] **v1.0-beta Commercial Audit Log Schema-B**：本地项目审计日志 schema 与只读聚合；追加写入口已在 Audit-I 收口
...
- [x] **v1.0-beta Project Retention Policy-J**：本地项目删除/保留策略 artifact 与 API；当前不实际删除项目、不接对象存储或数据库
- [x] **v1.0-beta Copyright Audit Hook-K**：版权/来源声明写操作接入本地审计日志；当前不接真实账号或不可篡改审计存储
- [x] **v1.0-beta MasterSetting Audit Hook-L**：设定轻编辑写操作接入本地审计日志；当前不接真实账号或不可篡改审计存储
...
- [x] **v1.0-beta Settings Local Smoke Checklist-Q**：设置抽屉展示本地冒烟清单；当前只生成 checklist，不主动执行请求
...
- **下一刀建议**：Object Storage Boundary Checklist-V 已收口；后续继续 v1.0-beta 本地优先商业化加固，可拆配额执行边界小刀；不要直接进入云端多租户或商业计费系统。
...
- **下一刀建议**：继续 v1.0-beta 本地优先商业化加固，可拆配额执行边界小刀；不要直接进入云端多租户或商业计费系统。
