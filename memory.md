...
2026-06-08 沙盘策略博弈结果可读化已完成第一版：`run_sandbox_round` / `get_sandbox_run` 的报告新增 additive `strategy_game_readout`，从现有 `agent_decision_advisory`、角色行动、主观记忆 delta、传播选择、代偿和章节钩子派生“谁在隐瞒 / 试探 / 误判 / 欺骗 / 反抗 / 临场改判”、为什么会这样，以及这一步改变了哪些事件、记忆、传播、代偿和章节素材；不新增持久 artifact，不改变 `agent_decision_advisory.json`、`sandbox_rounds.jsonl`、`subjective_memory_delta.json` 或 `run_scene` 默认行为。`WorldSandboxPage` 在“策略博弈读法”和“策略棋盘”之间新增“策略博弈结果总览”，优先展示可读结果账，再让用户进入密集策略棋盘。后端 focused test 覆盖 run report 与 persisted detail report，前端结构检查和 build 通过；完整验证结果见本轮 changelog。

2026-06-08 卷宗阅读正文内证据跳转已完成第一版：`dossier-reading` API 现在会从 `continuous_reading_chapter` 的段落证据引用和现有卷宗数据派生 `inline_evidence_anchors`，每个连续阅读段落可直接跳到角色个人卷、世界线状态/因果债、事件多视角或作者采纳台；`DossierReadingPage` 在正文段落内用“正文内证据锚点”按钮呈现这些跳转，保留旧折叠证据列表作为后查层。该刀只新增只读聚合字段和前端跳转，不新增持久 artifact，不改变 `continuous_reading_chapter`、`dossier-reading` 既有字段、阅读进度、长线卷或作者采纳契约。`test_dossier_reading_adds_inline_evidence_anchors_to_reading_sections` 和 `check:dossier-reading-ux` 先 RED 后转绿；完整验证结果见本轮 changelog。

2026-06-08 前端世界入口重组已完成第一版：`StoryEntryPage` 的故事卡和推荐世界主动作先进入 `#/anchor/<slug>` 世界房间，推荐动作保留为次动作；故事卡默认出口改为卷宗阅读、世界正史卷、主锚点卷、长线卷和世界沙盘，不再把机制档案作为默认故事卡出口。`WorldAnchorPage` 的“世界卷宗总览”新增世界正史卷、主锚点卷、角色个人卷、势力卷、事件多视角和跨事件长线卷入口，角色/势力缺失时降级到卷宗 tab；机制档案降为追溯链接。`AppShell` 顶栏持久露出正史卷和锚点卷，并修复两类独立卷页的 slug/worldline 识别。该刀只改前端入口/壳层/锚定页组织、样式和结构检查脚本，不新增后端 API，不改变路由兼容、阅读进度或 artifact 契约。`check:world-entry-navigation` 先 RED 后转绿；完整验证结果见本轮 changelog。

2026-06-08 卷宗阅读下一章接力台已完成第一版：`DossierReadingPage` 在连续阅读正文之后、阅读节奏/关联卷宗和“读完之后”出口之前新增“下一章接力台”。用户读完正文后，会先看到“本章留下什么 / 下一章要追什么 / 误会怎样发酵 / 从这里续写”四张卡，理解本卷怎样接到下一章、长线卷、下一轮沙盘或作者采纳台。该刀只改前端连续阅读接力层、样式和结构检查脚本，不新增后端 API，不改变 `dossier-reading` API、阅读进度、长线卷、作者采纳或 artifact 契约。`check:dossier-reading-ux` 先 RED 后转绿；完整验证结果见本轮 changelog。

2026-06-08 作者采纳台下一轮沙盘启动台已完成第一版：`AuthorAdoptionPage` 在确认入卷后、artifact 详情前新增“下一轮沙盘启动台”。用户确认正文入卷后，会先看到“下一轮事件 / 世界线状态 / 回流材料”三张卡，理解确认稿怎样喂回世界，并可直接“带确认稿回沙盘”、先读确认正文或追长线余波。该刀只改前端确认后回流理解层、样式和结构检查脚本，不新增后端 API，不改变作者采纳、确认入卷、沙盘 run 或 artifact 契约。`check:author-adoption-ux` 先 RED 后转绿；完整验证结果见本轮 changelog。

2026-06-08 沙盘结果下一步行动台已完成第一版：`WorldSandboxPage` 在“本轮因果回执”和“结果阅读顺序”之间新增“本轮之后先做什么”。跑完一轮后，用户会先看到“先读懂结果 / 带着因果续推 / 交给作者采纳”三条主路径，可分别跳到结果阅读顺序、把因果债带入下一轮运行台，或进入作者采纳台继续处理 brief、改写和入卷判断。该刀只改前端跑后行动中枢、样式和结构检查脚本，不新增后端 API，不改变 `POST /api/stories/<slug>/sandbox/run` 字段、路由、阅读进度、世界自演、作者采纳或 artifact 契约。`check:sandbox-runner-ux` 先 RED 后转绿；完整验证结果见本轮 changelog。

2026-06-08 沙盘策略误判回收台已完成第一版：`WorldSandboxPage` 在“势力反制账”和“下一轮暗线承接”之间新增“策略误判回收台”。当真实模型 advisory 产出策略互动时，用户会先看到“误判从哪里来 / 谁被误导 / 证据在哪里 / 下一轮怎么回收”四张回收卡，前三张分别追策略棋盘、角色行动链或长线卷，最后一张可把误判回收直接放入运行台，并在下一轮草稿中标明来源为“误判回收”。该刀只改前端跑后误判回收理解、样式、回填状态和结构检查脚本，不新增后端 API，不改变 `POST /api/stories/<slug>/sandbox/run` 字段、路由、阅读进度、世界自演、作者采纳或 artifact 契约。`check:sandbox-runner-ux` 先 RED 后转绿；完整验证结果见本轮 changelog。
