...
| v0.9.4 | Advanced Runner Evaluation Spike：评估 LangGraph 局部 runner、OASIS/CAMEL 可选 runner | 进行中：Trigger-A 已完成 |
...
**测试基线**：`pytest -q` → **641 passed**（2026-06-01，v0.9.4 Advanced Runner Evaluation Trigger-A 后完整回归通过）；`engine/ui` 执行 `pnpm run build` 通过。
...
### v0.9.4 Advanced Runner Evaluation Spike（进行中）

Trigger-A 已新增只读触发报告，不接 LangGraph、OASIS 或 CAMEL：
...
- 当前不接 LangGraph、OASIS、CAMEL；不写 artifact，不替换现有 runner，不改 `run_scene` 默认行为。
...
| v0.9.1-v0.9.4 | provider/cost、MasterSetting Lite、Graph Memory spike、Advanced Runner spike（v0.9.4 Trigger-A 已完成，下一步 Probe-B） |
