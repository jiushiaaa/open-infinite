...
沙盘结果承接台新增“本轮承接来源”：启动一轮时会把回填来源或手写事件、事件草稿和干预边界保存为前端 `lastRoundLaunchReceipt`，跑后在“本轮已发生”内继续显示，并提供回到运行台或读结果顺序动作。该层只保留前端上下文，不新增 API，不改变 sandbox run artifact。
