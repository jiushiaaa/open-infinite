...
`--json` 输出与 HTTP closeout 同构的 `story_slug/version/completion_status/actions/closeout`；`--require-ready` 在 `closeout.can_close_alpha=false` 时以退出码 1 失败。该命令只读扫描项目工作台，不写 artifact、不执行 action、不改变 `run_scene` 默认行为。
