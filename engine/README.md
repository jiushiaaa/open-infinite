...
| v0.9.0-alpha | Long Novel Creation Loop：上传、记忆、分支运行、审计、选择世界线、导出 | 进行中：Export / Checklist / Hint / Resume Job / Selection 已收口 |
...
| v0.9.0-alpha | 长篇产品闭环 | 进行中：Export / Checklist / Hint / Resume Job / Selection 已走通，完整主链路仍是 alpha |
| v0.9.1-v1.0-beta | 增强与商业化 | provider/cost、MasterSetting、图记忆/advanced runner 评估，以及商业级加固 |

**测试基线**：`pytest -q` → **604 passed**（2026-05-31，v0.9.0-alpha Worldline Selection Persistence 子刀后完整回归通过）；`engine/ui` 执行 `pnpm run build` 通过。
...
| v0.9.0-alpha | 长篇共创闭环：上传 -> 记忆 -> 分支运行 -> 审计 -> 选择世界线 -> 导出（进行中：Export / Checklist / Hint / Resume Job / Selection 已收口） |
