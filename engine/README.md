...
| v0.9.1-v1.0-beta | 增强与商业化 | provider/cost、MasterSetting、图记忆/advanced runner 评估、商业化范围复核、本地部署就绪、云端持久化边界、账号/项目空间边界、审计追加策略、项目保留策略、关键写操作审计钩子、发布前检查、版权审批准备度、部署观测清单、认证边界、对象存储边界和配额执行边界 |

**测试基线**：`pytest -q` → **706 passed**（2026-06-01，v1.0-beta Quota Enforcement Boundary Checklist-W 收口后完整回归通过）；`engine/ui` 执行 `pnpm run build` 通过。
...
四个子刀已新增只读 MasterSetting 工作台汇总、前端展示、后端白名单轻编辑与前端最小写控件，不改变导入或 runner：
...
- 前端「设定工作台」可编辑作品名、题材、世界规则、力量限制、禁用设定，保存后先本地更新面板并显示成功提示，再刷新项目工作台。
- 保存前备份 `backups/<timestamp>/memory/master_setting.yaml`，保存后写 `memory/master_setting_update_report.json`。
- 当前不编辑人物、时间线、伏笔、章节摘要，不同步 `world.yaml`，不做完整作者工作台。
- 收口归档见 `../docs/completed/v0.9.2-master-setting-workspace-lite.md`；下一步仅在长篇召回不足时评估 v0.9.3 图记忆。

### v0.9.3 Graph Memory Evaluation Spike（已收口）

本版本已新增只读触发报告与检索 probe，不接任何图数据库或外部服务：

- `GET /api/stories/<slug>/graph-memory-evaluation`
- 读取 `import_report.json`、`memory/canon_ledger.jsonl`、`memory/entity_aliases.yaml` 与 `memory/consistency_report.json`。
- 返回 `status=not_triggered|monitor|triggered`、章节数、字数、账本数、别名数、审计风险、触发原因、阈值和中文下一步。
- `GET /api/stories/<slug>/retrieval-probes`
- 从 canon ledger 与 entity aliases 自动生成代表性查询样本，复用现有 `retrieve_context()` 检查期望 source、item 和实体是否命中。
- 返回 `pass|weak|insufficient_samples`、命中率、top sources、失败样例和中文建议。
- 坏 slug 返回 400，缺故事返回 404。
- 当前不接 Zep、图数据库、GraphRAG、embedding、向量库或 reranker；不写新 run artifact，不替换 BM25 / canon ledger / entity aliases。

### v0.9.4 Advanced Runner Evaluation Spike（已收口）

本版本已新增只读触发报告与 runner probe，不接 LangGraph、OASIS 或 CAMEL：

- `GET /api/runs/<run_id>/advanced-runner-evaluation`
- 读取 `runner_state_execution_report.json`、分支 `multi_agent_trace.json` 与 `emergence_nodes.json`。
- 返回 `status=not_triggered|insufficient_data|triggered`、状态执行候选 backlog、trace warning、私域复杂度、high-value emergence、触发原因和中文下一步。
- `GET /api/runs/<run_id>/advanced-runner-probes`
- 把状态执行候选、trace 质量和涌现节点拆成可复现 probe，返回 `pass|weak|insufficient_data`、命中数、失败样例和中文建议。
...
本版本已新增本地项目审计日志 schema 与只读聚合；追加写入口已在 v1.0-beta Audit-I 单独收口：
...
本版本已新增本地部署就绪清单，不执行真实部署：

- `GET /api/settings/deployment-readiness`
- 返回 `version=v1.0-beta-local-deployment-readiness-f`、`readiness`、`environment`、`checks`、`api_smoke_plan`、`run_steps`、`verification_steps`、`observability`、`warnings` 和中文 `next_steps`。
- `readiness.external_services_required=false`，表示本地启动不要求真实外部模型、云端对象存储、账号系统或计费系统。
- `environment` 仅展示密钥是否存在与脱敏尾号，不返回明文密钥或环境变量名。
- 静态资源或本地数据目录缺失时返回 `status=attention`，不抛 500。
- 当前不接云端托管、对象存储、多用户账号、商业计费、外部监控、告警系统或自动部署脚本。

### v1.0-beta Cloud Persistence Boundary-G（已收口）

本版本已新增云端持久化迁移边界，不执行真实迁移：

- `GET /api/settings/cloud-persistence-boundary`
- 返回 `version=v1.0-beta-cloud-persistence-boundary-g`、`migration`、`local_inventory`、`resource_map`、`retention_policy`、`readiness_checks`、`deferred_actions`、`warnings` 和中文 `next_steps`。
- `migration.mode=not_started`、`external_services_required=false`，表示当前只做本地 artifact 到未来平台资源的边界报告。
- `resource_map` 覆盖 `source_raw/`、`source/`、`memory/`、`canon/holdout_private/`、`outputs/run_*/`、`_ingest_sessions/` 和 `selected_worldline.json`。
...
本版本已新增项目删除/保留策略，不执行真实删除：

- `GET /api/stories/<slug>/retention-policy`
- `POST /api/stories/<slug>/retention-policy`
- 本地写入 `memory/project_retention_policy.json`，字段覆盖项目、上传原文、生成产物、holdout、审计日志和 ingest 分片的保留口径。
- 写入策略后追加 `retention_policy_reviewed` 审计事件到 `memory/project_audit_log.jsonl`。
...
本版本已把设定轻编辑写操作接入本地项目审计日志：

- `update_master_setting()` 在成功写入 `memory/master_setting.yaml` 与 `memory/master_setting_update_report.json` 后追加 `master_setting_updated` 审计事件。
- 审计事件写入 `memory/project_audit_log.jsonl`，并带上 `artifact_path`、`report_path` 与 `changed`。
- 审计追加白名单新增 `master_setting_updated`。
- `GET /api/stories/<slug>/audit-log` 会继续聚合该事件。
- 现有 `POST /api/stories/<slug>/master-setting` 错误语义保持不变。
- 当前不接真实账号、团队空间、认证 provider、云端不可篡改审计存储、对象存储、数据库或队列。

### v1.0-beta Worldline Selection Audit Hook-M（已收口）

本版本已把世界线选择写操作接入本地项目审计日志：

- `select_worldline()` 在成功写入 `selected_worldline.json` 后追加 `worldline_selected` 审计事件。
- 审计事件写入导入项目的 `memory/project_audit_log.jsonl`，并带上 `artifact_path`、`run_id`、`branch_id` 与 `branch_label`。
...
- `GET /api/settings/quota-enforcement-boundary` 返回 `version=v1.0-beta-quota-enforcement-boundary-checklist-w`、`summary`、`checks`、`warnings` 与 `next_steps`。
...
- 当前只读定义配额执行前置边界，不拦截生成/导入/导出/视觉生成请求，不写 quota state，不接真实计费、账单、余额、支付、云端监控或告警；不改变 `run_scene`。
...
| v1.0-beta Audit Hook-L | 设定轻编辑审计钩子（已收口） |
| v1.0-beta Audit Hook-M | 世界线选择审计钩子（已收口） |
