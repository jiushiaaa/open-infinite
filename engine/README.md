...
| v0.9.0-alpha | Long Novel Creation Loop：上传、记忆、分支运行、审计、选择世界线、导出 | 进行中：Export / Checklist / Hint 已收口 |
...
| v0.9.0-alpha | 长篇产品闭环 | 进行中：Export / Checklist / Hint 已走通，完整主链路仍是 alpha |
...
前端还会显示 `creation_loop.recommended.continue_hint` 作为 CLI 续写入口，例如 `lne resume continue <run_id> --branch <branch_id> --mock`。当前未新增 HTTP resume job。
...
| v0.9.0-alpha | 长篇共创闭环：上传 -> 记忆 -> 分支运行 -> 审计 -> 选择世界线 -> 导出（进行中：Export / Checklist / Hint 已收口） |
