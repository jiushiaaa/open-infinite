...
- `POST /api/stories/<slug>/sandbox/run`：输入 `major_event` 与可选 `worldline_id`，生成本地单轮沙盘；可选 `intervention_content` / `intervention_target` 会先读取《天命书》编译为本轮干预约束。
...
- `outputs/<run_id>/intervention_constraint.json`：当本轮沙盘带干预文本时写入，记录天命书编译出的法则吸收、分支轴、因果债和普通干预不覆盖根天命书边界。
