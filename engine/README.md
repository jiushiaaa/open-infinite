...
| v0.9.2 | MasterSetting Workspace Lite：项目级设定/人物/时间线/道具/伏笔/章节摘要工作台 | 进行中：Summary-A 已收口 |
...
**测试基线**：`pytest -q` → **627 passed**（2026-06-01，v0.9.2 MasterSetting Workspace Summary-A 后完整回归通过）；`engine/ui` 执行 `pnpm run build` 通过。
...
第一刀已新增只读 MasterSetting 工作台汇总，不改变导入或 runner：
...
- 当前只读，不写 artifact，不做完整作者工作台；前端面板与最小轻编辑留后续子刀。
...
| v0.9.1-v0.9.4 | provider/cost、MasterSetting Lite、Graph Memory spike、Advanced Runner spike（v0.9.2 Summary-A 已收口，后续前端面板/轻编辑） |
